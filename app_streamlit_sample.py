...
def fetch_suggestions(condition, symptoms, age_group, risk_level):
    ...
    # Loop through each symptom (main suggestions only)
    for s in symptoms[:2]:  # limit to 2 symptoms to avoid repetition
        s = s.lower()
        matches = df[
...
if st.button("Run Prediction"):
    ...
    df = pd.DataFrame([
        ("Systolic BP", bp_sys, bp_sys_ref, bp_sys_r, "Hypotension" if bp_sys_r == "Low" else ""),
        ("Diastolic BP", bp_dia, bp_dia_ref, bp_dia_r, ""),
        ("Fasting Sugar", sugar_f, sugar_f_ref, sugar_f_r, "Hypoglycemia" if sugar_f_r == "Low" else ""),
        ("Random Sugar", sugar_r, sugar_r_ref, sugar_r_r, ""),
        ("BMI", bmi, "—", "—", ""),
        ("Heart Rate", hr, "—", "—", "")
    ], columns=["Parameter", "Value", "ReferenceRange", "Risk", "Condition"])
    icons = {"Low":"🔵","Normal":"🟢","Moderate":"🟡","High":"🔴","Unknown":"⚪","—":"⚪"}
    df["Icon"] = df["Risk"].map(lambda x: icons.get(x, "⚪"))
    st.dataframe(df[["Parameter","Value","Icon","Risk","Condition","ReferenceRange"]], use_container_width=True)
